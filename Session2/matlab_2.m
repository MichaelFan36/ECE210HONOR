...
% lecture slides.
% 
% In 3 separate plots, plot the sum for n  = 10, 100, 1000.
...
sum10 = 0;
for i = 1:10
    sum10 = sum10 + 4/pi*((sin(2*pi*(2*i-1)*t))/(2*i-1));
end
plot(t,sum10)
xlabel('t')
ylabel('sum10')
...
sum100 = 0;
for i = 1:100
    sum100 = sum100 + 4/pi*((sin(2*pi*(2*i-1)*t))/(2*i-1));
end
plot(t,sum100)
xlabel('t')
ylabel('sum100')
...
sum1000 = 0;
for i = 1:1000
    sum1000 = sum1000 + 4/pi*((sin(2*pi*(2*i-1)*t))/(2*i-1));
end
plot(t,sum1000)
xlabel('t')
ylabel('sum1000')
...
for i = 1:size(input_image, 1)/4
    for j = 1:size(input_image, 2)/4
        output_image(i , j) = input_image(4*i, 4*j);
    end
end
imshow(output_image)
